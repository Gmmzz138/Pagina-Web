...
/* 4. Lógica de generación del collage e interacción */
...
/* 5. Función del Modal (Plantilla única para todas las ciudades) */
function abrirModal(nombre) {
    const modal = document.getElementById('modalDetalle');
    
    // Si la ciudad existe en infoDetallada, usa sus datos. Si no, usa un texto genérico.
    const datos = infoDetallada[nombre] || { 
        desc: "Descubre la magia de este destino con nuestros guías expertos. Historia y paisajes te esperan.", 
        precio: "Consultar" 
    };
...
// 6. Crear o recuperar la lista de favoritos desde el almacenamiento del navegador
...
function abrirModal(nombre) {
    ...
    if (misFavoritos.includes(nombre)) {
        btnFav.innerText = "Quitar de Favoritos";
        btnFav.style.backgroundColor = "#ff4d4d"; // Rojo si ya está
    } else {
...
function gestionarFavorito(nombre) {
    const index = misFavoritos.indexOf(nombre);
    let mensajeTexto = "";
